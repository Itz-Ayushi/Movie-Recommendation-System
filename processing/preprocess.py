...
def cosine_similarity_custom(matrix):
    ...
    similarity_matrix = dot_product / (norm_matrix + 1e-10)  # avoid division by zero
    return similarity_matrix

def vectorise(new_df, col_name):
    cv = CountVectorizer(max_features=5000, stop_words='english')
    vec_tags = cv.fit_transform(new_df[col_name]).toarray()
    sim_bt = cosine_similarity_custom(vec_tags)
    return sim_bt
...
def knn_from_scratch(vec_matrix, index, k=5):
    target_vec = vec_matrix[index]
    similarities = []

    for i, vec in enumerate(vec_matrix):
        if i == index:
            continue  # skip the movie itself
        dot = np.dot(target_vec, vec)
        norm_target = np.linalg.norm(target_vec)
        norm_vec = np.linalg.norm(vec)
        sim = dot / (norm_target * norm_vec + 1e-10)
...
def recommend(new_df, movie, col_name='tags', k=10):
    cv = CountVectorizer(max_features=5000, stop_words='english')
    vec_tags = cv.fit_transform(new_df[col_name]).toarray()
    ...
    try:
        ...
        return [], []  # Handle if movie not found

    # Get top K neighbors (excluding the movie itself)
    top_k = knn_from_scratch(vec_tags, movie_idx, k)

    rec_movie_list = []
    rec_poster_list = []

    for idx, _ in top_k:
        rec_movie_list.append(new_df.iloc[idx]['title'])
